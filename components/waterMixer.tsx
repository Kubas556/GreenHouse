...
function WaterMixer(props: IWaterMixer) {
  ...
  const [fertiliserValue, setFertiliserValue] = useState<number>(
    (100 / props.defaults.total) * props.defaults.fertiliser
  ); // procenta
  const [waterValue, setWaterValue] = useState<number>(
    (100 / props.defaults.total) * (props.defaults.total - props.defaults.fertiliser)
  ); // procenta
  ...
  function onMouseMove(e: MouseEvent) {
    if (e.nativeEvent.buttons === 1) {
      e.preventDefault();
      const angle = calculateAngle(e.nativeEvent.offsetX - 337 / 2, e.nativeEvent.offsetY - 337 / 2 - 13);
      setFertiliserValue((100 / 360) * angle);
      const fert = (100 / 360) * angle;
  ...
  function callChangeEvent(fertiliser: number, total: number) {
    props.onChange({
      ...
      ratio:
        fertiliser < 100 - fertiliser
          ? // tslint:disable-next-line:prefer-template
            '1.00:' + (fertiliser === 0 ? '0.00' : ((100 - fertiliser) / fertiliser).toFixed(2))
          : // tslint:disable-next-line:prefer-template
            (100 - fertiliser === 0 ? '0.00' : (fertiliser / (100 - fertiliser)).toFixed(2)) +
            ':1.00' /*(fertiliser < (100-fertiliser)?"1.00:"+(fertiliser==0?"0.00":((100-fertiliser)/fertiliser).toFixed(2)):((100-fertiliser)==0?"0.00":(fertiliser/(100-fertiliser)).toFixed(2))+":1.00")*/,
    });
  }
  // @ts-ignore
  function onTouchMove(e: TouchEvent<HTMLDivElement>) {
    ...
    if (touchArea.current) {
      const testX =
        ...
        (touchArea.current.getBoundingClientRect().left + window.scrollX) -
        componentWidth / 2;
      const testY =
        e.nativeEvent.targetTouches[0].pageY -
        (touchArea.current.getBoundingClientRect().top + window.scrollY) -
        componentHeight / 2;

      const angle = calculateAngle(testX, testY);
      setFertiliserValue((100 / 360) * angle);
      const fert = (100 / 360) * angle;
      callChangeEvent(fert, totalValue);
    }
  ...
  return (
    <div>
      <div
        ref={touchArea}
        style={{ touchAction: 'none', width: componentWidth, height: componentHeight }}
      ...
      >
        ...
        >
          <defs>
            <style>
              {`.cls-1-watermixer,.cls-10-watermixer,.cls-12-watermixer,.cls-2-watermixer,.cls-7-watermixer,.cls-8-watermixer,.cls-9-watermixer{fill:none;}.cls-1-watermixer,.cls-2-watermixer{stroke:${
                props.theme ? 'white' : 'black'
              };}.cls-1-watermixer,.cls-10-watermixer,.cls-2-watermixer,.cls-7-watermixer,.cls-8-watermixer,.cls-9-watermixer{stroke-miterlimit:10;}.cls-1-watermixer{stroke-opacity:0.49;}.cls-2-watermixer{opacity:0.49;}.cls-3-watermixer{font-size:22px;fill:${
                props.theme ? 'white' : 'black'
              };font-family:"Roboto", "Helvetica", "Arial", sans-serif;}.cls-4-watermixer{letter-spacing:-0.03em;}.cls-5-watermixer{letter-spacing:0.01em;}.cls-6-watermixer{letter-spacing:-0.01em;}.cls-7-watermixer,.cls-8-watermixer{stroke:#18b8d4;}.cls-7-watermixer,.cls-9-watermixer{stroke-width:18px;}.cls-10-watermixer,.cls-8-watermixer{stroke-width:40px;opacity:0.2;}.cls-10-watermixer,.cls-9-watermixer{stroke:#fb6544;}.cls-11-watermixer{fill:#2196f3;}`}
            </style>
          </defs>
          <g id="Vrstva_2" data-name="Vrstva 2">
            <g id="Vrstva_4" data-name="Vrstva 4">
              ...
              <text className="cls-3-watermixer" transform="translate(47.43 30.09)">
                ...
                <tspan className="cls-6-watermixer" x="49.35" y="0">
                  v
                </tspan>
                <tspan x="59.71" y="0">
                  o
              ...
              <circle
                className="cls-7-watermixer"
                cx="178.77"
                cy="215.28"
                r="119.82"
                strokeDasharray={maxProgress}
                strokeDashoffset={blueProgress}
              />
              <circle
                className="cls-8-watermixer"
                cx="178.77"
                cy="215.28"
                r="119.82"
                strokeDasharray={maxProgress}
              ...
              />
              <circle
                className="cls-9-watermixer"
                cx="178.77"
                cy="215.28"
                r="119.82"
                strokeDasharray={maxProgress}
                strokeDashoffset={redProgress}
              />
              <circle
                className="cls-10-watermixer"
                cx="178.77"
                cy="215.28"
                r="119.82"
                strokeDasharray={maxProgress}
              ...
              />
              <line
                className="cls-1-watermixer"
                x1="13.02"
                y1="215.28"
                x2="178.77"
                y2="215.28"
                style={{ transform: `rotate(${90 - lineAngle}deg)` }}
              />
              <circle
                className="cls-11-watermixer"
                cx="13.02"
                cy="215.28"
                r="13.02"
                style={{ transform: `rotate(${90 - lineAngle}deg)`, cursor: 'grab' }}
              />
      ...
      <div>
        {componentWidth < 300 ? null : (
          <WaterMixerTable
            water={(totalValue / 100) * waterValue}
            fertiliser={(totalValue / 100) * fertiliserValue}
