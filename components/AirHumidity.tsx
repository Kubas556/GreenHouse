...
const AirHumidity = (props: IAirHumudity) => {
  ...
  function setValue(val: number) {
    ...
    if (percentageLevel.current) {
      percentageLevel.current.setAttribute('d', `M107.87,${value}V178H-2.5V${value}Z`);
    }
  ...
  anime({
    ...
    update() {
      if (wave2.current) {
        wave2.current.setAttribute('transform', `translate(${animX1.x},${currentY - waveGap})`);
      }
    },
  });
  anime({
    ...
    update() {
      if (wave1.current) {
        wave1.current.setAttribute('transform', `translate(${animX2.x},${currentY - waveGap})`);
      }
    },
  });
  ...
  return (
    <div style={{ width: componentWidth, height: componentHeight }}>
      <style>
        {`.cls-2-vlh-vz{font-size:12px;fill:'${props.theme ? 'white' : 'black'}';font-family: 'Inter', sans-serif;}`}
      </style>
      <svg viewBox="0 0 225.68 211.24" style={{ width: componentWidth }}>
        <defs>
          <style>
            {`.cls-1-vlh-vz,.cls-7-vlh-vz,.cls-8-vlh-vz{fill:none;}.cls-2-vlh-vz{font-size:12px;fill:${
              props.theme ? 'white' : 'black'
            };}.cls-3-vlh-vz{clip-path:url(#clip-path-vlh-vz);}.cls-4-vlh-vz,.cls-5-vlh-vz,.cls-6-vlh-vz{fill:#18b8d4;}.cls-5-vlh-vz{opacity:0.2;}.cls-6-vlh-vz{opacity:0.4;}.cls-7-vlh-vz,.cls-8-vlh-vz{stroke:${
              props.theme ? 'white' : 'black'
            };stroke-miterlimit:10;}.cls-7-vlh-vz{stroke-width:0.25px;}.cls-8-vlh-vz{stroke-opacity:0.49;}`}
          </style>
        ...
        <g id="Vrstva_2" data-name="Vrstva 2">
          <g id="Vrstva_2-2" data-name="Vrstva 2">
            <text className="cls-2-vlh-vz" ref={percentageText} transform="translate(168.67 92.37)">
              20%
            </text>
