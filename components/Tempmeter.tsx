...
function Tempmeter(props: ITempmeter) {
  ...
  useEffect(() => {
    const rangeVal = textValue >= 0 ? textValue + 50 : 50 - Math.abs(textValue);
    const y: number = height - ((height / 100) * rangeVal - 8);
  ...
  useEffect(() => {
    ...
    setCurrentAnim(
      anime({
        ...
        round: 1,
        update() {
          if (textAnim.text > minTemp && textAnim.text < maxTemp) setTextValue(textAnim.text);
        },
      })
    );
  }, [currentTemp]);
  ...
  return (
    <div id={'tempmeterContainer'}>
      ...
      <svg id={'tempmeter'} xmlns="http://www.w3.org/2000/svg" width={'110'} viewBox="0 0 83.45 252.84">
        <defs>
          <style>
            {`.cls-1-termostat,.cls-3-termostat,.cls-5-termostat{fill:none;stroke-miterlimit:10;}.cls-1-termostat{stroke:#4950f5;stroke-linecap:round;stroke-width:7px;}.cls-2-termostat{fill:#4950f5;}.cls-3-termostat,.cls-5-termostat{stroke:${
              props.theme ? 'white' : 'black'
            };stroke-width:0.25px;}.cls-4-termostat{font-size:12px;fill:${
              props.theme ? 'white' : 'black'
            };}.cls-5-termostat{stroke - dasharray:0.99 0.99;}`}
          </style>
        </defs>
        ...
        <g id="Vrstva_2" data-name="Vrstva 2">
          <g id="Vrstva_2-2" data-name="Vrstva 2">
            ...
            <text className="cls-4-termostat" transform="translate(6.31 211.27)">
              -50
            </text>
            <text className="cls-4-termostat" transform="translate(0 10.31)">
              50
            </text>
            ...
            <text ref={text} className="cls-4-termostat" transform={`translate(50.59 ${minHeight - 2.77})`}>
              {textValue}°C
            </text>
            <text className="cls-4-termostat" transform="translate(6.3 110.81)">
              0
            </text>
